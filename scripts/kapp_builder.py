...
from scripts.FVA_integration.fvfa import fva_solve_faster
from scripts.FVA_integration.utils import cobra_to_fva_problem
...
def create_enzyme_info_dataframe(enzymes_df, fluxomics_df, substrates_df, sequence_df):
    ...
    for flux_col in flux_columns:
        ...
        # Determine condition number for matching FVA columns
        condition_num = flux_col.split('flux_cond')[-1]
        fva_lower_col = f'FVA_lower_cond{condition_num}'
        fva_upper_col = f'FVA_upper_cond{condition_num}'

        # == Fluxomics info ==
        # Merge with specific flux condition
        flux_subset = fluxomics_df[['rxn_id', flux_col, fva_lower_col, fva_upper_col]].copy()
        flux_subset.rename(columns={
            flux_col: 'flux_value',
            fva_lower_col: 'FVA_lower',
            fva_upper_col: 'FVA_upper'
        }, inplace=True)
